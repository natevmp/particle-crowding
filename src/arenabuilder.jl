...
function evolveArena(arena::Arena, steps::Int; plotsteps=false, animator::Union{Animation, Nothing}=nothing)
    ...
    for t in 1:steps
        ...
        if plotsteps||(animator!==nothing)
            ...
            # nCollisions = length(collidedCellsList_id)/2
            # println("collisions in step: ", nCollisions)
        else
            # collider!(arena)
            collidedCellsList_id = collider!(arena)
            # nCollisions = length(collidedCellsList_id)/2
        ...
        snapshotCells(arena, t, posTime_id_t_dim, velTime_id_t_dim)
        snapshotCells!(arena, t, cells_T_ID)
    end
...
function plotArena(arena::Arena, collidedCellsList_id::Union{Vector{Int}, Nothing}; displayPlot=true, animator::Union{Animation, Nothing}=nothing)
