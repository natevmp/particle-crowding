...
"""Fix potentially overlapping cells in an arena."""
function fixArenaOverlaps!(arena::Arena, scanLimit::Int=20)
    ...
    while true
        ...
        if counter > scanLimit
            error("could not resolve overlaps in ", scanLimit, " passes. Try a different cell configuration or a smaller number of cells.")
        end
    end
